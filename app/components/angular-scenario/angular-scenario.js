...
angular.scenario.dsl('element', function() {
  ...
  chain.tap = function(){
    ...
    return this.addFutureAction('element ' + this.label + ' tap', function($window, $document, done) {
        $window.Hammer($document.find($this.label)[0]).trigger('tap',{target:$document.find($this.label)[0]});
        done();
    });
  };
  ...
  chain.hold = function(){
      var $this = this;
    return this.addFutureAction('element ' + this.label + ' hold', function($window, $document, done) {
        $window.Hammer($document.find($this.label)[0]).trigger('hold',{target:$document.find($this.label)[0]});
        done();
    });
  };
