...
angular.module('conductivEcatalogApp')
		.factory('fileSync', function (catalog, customer, order, productStyles, assortment, $routeParams, s3directoryService, $location, $timeout, $rootScope, Environment, authentication,QueueManager,OfflineManager) {
			...
			function addfiles(foldersToCheckForUpdates, aggregatedRecentlyChangedFiles, onSuccess, loaderStartFrom) {
				...
				$.when.apply(null, deferredFolderListResults).done(function () {
					if (aggregatedRecentlyChangedFiles.length > 0) {
						...
						//catalog.complteBar() is temporal solution until we
						//can handler the .progress() without notify()
						var complteBar = catalog.completeBar();
						complteBar.progress(function () {
							$rootScope.progressBarStatus += fractionOfDownloadProgress;
							setTimeout(function () {
								//All Setup, starting the Queue Manager...
								QueueManager.connectionCheck();
			...
			return {
				syncApplication: function (onSuccess) {
					...
					productStyles.saveAllProductStyles();
					//Validations...
					...
					//Now let's Syncing All Catalogs...
					catalog.persistCatalogsListOffline().then(function(catalogs){
						//Saving Catalog Data...
						_.each(catalogs,function(catalogObj){
							catalog.persistMasterCatalogOffline(catalogObj.id).then(function(){
								catalog.persistMasterCatalogPagesListOffline(catalogObj.id);
							});
						});
						...
						assortment.saveAssortments().then(function(assortments){
							...
							order.saveAllOrdersOffline().then(function(orders){
								...
								if (Environment.isIOS) {
									...
									//Savin Master Catalog Image Files...
												...
												assortmentSyncDeferred.progress(function () {
													...
													$timeout(function () {
														if(!$rootScope.$$phase) {
															...
															var fractionCalc = 0.30;
															$rootScope.progressBarStatus = 50;
