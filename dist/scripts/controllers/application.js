...
angular.module('conductivEcatalogApp').controller('ApplicationCtrl', function ($scope, productStyles, assortment, catalog, $rootScope, $filter, $location, authentication, $window, $timeout, fileSync, httpCapi, SyncManager, OfflineManager, QueueManager, Environment) {
    ...
    $rootScope.browseCatalog = function (catalogId) {
        ...
        if (catalog.isDownloaded(catalogId)) {
            ...
            fileSync.syncCatalog(catalogId, function () {
                var catalogId = angular.copy($rootScope.openingCatalogId);
...
//                  $rootScope.$broadcast('event:network-connectivity-assortments', true);
    ...
    // Catalog operations start
    if (isUserLoggedIn) {
        if ($rootScope.masterCatalogs === undefined || $rootScope.masterCatalogs === null) {
            catalog.getMasterCatalogs().then(function (catalogs) {
                $rootScope.masterCatalogs = catalogs;
            });
        }
    }
    // Catalog operations end
